...
module.exports = {
	...
	errors: function(item, errors) {
		...
		//auth_slug		Required
		if(this.validate().isSet(item.auth_slug) 
		&& this.validate().isEmpty(item.auth_slug)) {
			errors.auth_slug = this.INVALID_EMPTY;
		}
		
		// auth_permissions		Required
		if(this.validate().isSet(item.auth_permissions) 
		&& this.validate().isEmpty(item.auth_permissions)) {
			errors.auth_permissions = this.INVALID_SET;
		}
		
		//confirm			NOT IN SCHEMA
		if(((!this.validate().isEmpty(item.auth_password))
		|| (!this.validate().isEmpty(item.confirm)))
